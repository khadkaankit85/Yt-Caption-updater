...
async function startUpdatingVideoTitle(res: Response): Promise<void> {
    ...
    try {
        ...
        const updateResponse = await updateVideoTitle(service, videoId, newTitle, categoryId);
        ...
        res.status(500).send('Error updating video title: ' + JSON.stringify(error));
    }
}

async function getVideoDetails(service: youtube_v3.Youtube, videoId: string): Promise<youtube_v3.Schema$Video | null> {
...
async function getVideoViewCount(videoId: string, apiKey: string): Promise<number> {
    const url = `${mainUrlYt}videos?part=statistics&id=${videoId}&key=${apiKey}`;
    try {
        const response = await fetch(url);
        const data: VideoStatisticsResponse = await response.json() as VideoStatisticsResponse;
        return data.items && data.items.length > 0 ? parseInt(data.items[0].statistics.viewCount) : 0;
    } catch (error) {
...
async function getValidCategories(service: youtube_v3.Youtube): Promise<Map<string, string>> {
...
async function updateVideoTitle(service: youtube_v3.Youtube, videoId: string, newTitle: string, categoryId: string): Promise<youtube_v3.Schema$Video> {
